...
def setup_enable_pin(printer, pin):
    ...
    mcu_enable.setup_start_value(1,1)
    enable = pin_params['class'] = StepperEnablePin(mcu_enable, 1)
    return enable
...
class EnableTracking:
    ...
    def is_stepper(self, stepper_name):
        return self.stepper.get_name().upper() == stepper_name.upper()
...
class PrinterStepperEnable:
    def __init__(self, config):
        ...
        gcode.register_command("F101", self.cmd_F101)
    ...
    def motor_off(self):
        ...
        toolhead.dwell(DISABLE_STALL_TIME)
    def motor_off_retain_one(self, stepper_name):
        toolhead = self.printer.lookup_object('toolhead')
        toolhead.dwell(DISABLE_STALL_TIME)
        print_time = toolhead.get_last_move_time()
        for el in self.enable_lines.values():
            if el.is_stepper(stepper_name):
                logging.info(f"motor_off_retain_one {stepper_name}")
            else:
                el.motor_disable(print_time)
        self.printer.send_event("stepper_enable:motor_off", print_time)
        toolhead.dwell(DISABLE_STALL_TIME)
    ...
    def cmd_M18(self, gcmd):
        # Turn off motors
        self.motor_off()
    def cmd_F101(self, gcmd):
        # Turn off motors, but retain stepper_a on
        params = gcmd.get_command_parameters()
        if len(params) != 2:
            gcmd.respond_info(f'F101: Invalid parameters count {len(params)-1}')
            return
        # Get stepper name from command parameters
        stepper_name = f"stepper_{gcmd.get_raw_command_parameters()}".lower()
        # Check stepper name is valid
        if stepper_name not in self.enable_lines:
            gcmd.respond_info('F101: Invalid stepper "%s"'
                              % (stepper_name,))
            return
        self.motor_off_retain_one(stepper_name)
